...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['NOT_DIAMOND_LOG'] = undefined;
    });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['NOT_DIAMOND_LOG'] = 'debug';
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['NOT_DIAMOND_LOG'] = 'not a log level';
      const client = new NotDiamond({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'NOT_DIAMOND_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['NOT_DIAMOND_BASE_URL'] = undefined;
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['NOT_DIAMOND_BASE_URL'] = 'http://localhost:5000/env';
      const client = new NotDiamond({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
